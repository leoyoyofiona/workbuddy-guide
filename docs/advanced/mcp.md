...
本页不重复放桌面截图。先用上面的授权示意图理解数据流，再回到[看懂桌面](/start/chat/)页面核对当前客户端的实际入口；截图版本变化时，以当前提示为准。

## 连接器案例的 5 个暂停点

| 阶段 | 检查点 | 你必须确认什么 | 不满足时 |
| --- | --- | --- | --- |
| 1. 识别入口 | 当前客户端入口 | 能力名称、授权范围和账号。 | 先看当前提示，不照搬旧截图。 |
| 2. 写只读计划 | 查询说明 | 数据源、查询条件、时间范围和预计字段。 | 不执行写入，不继续。 |
| 3. 检查权限 | 权限弹窗 | 是只读、创建、修改还是发送；是否产生 Credits。 | 降为只读或停止。 |
| 4. 看结果 | 结果区 | 返回内容是否带来源、是否含敏感信息。 | 脱敏、缩小范围、重新查询。 |
| 5. 人工批准 | 发送/写入前 | 外发、修改或付费动作的草稿和目标对象。 | 不发送、不写入。 |
